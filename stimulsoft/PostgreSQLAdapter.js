...
exports.process = function (command, onResult) {
  var end = function (result) {
    try {
      ...
      result.adapterVersion = '2022.1.2';
      onResult(result);
    } catch (e) {}
  };
  ...
  try {
      ...
      for (var recordIndex in recordset.rows) {
        ...
        for (var columnName in recordset.rows[recordIndex]) {
          ...
          if (recordset.rows[recordIndex][columnName] instanceof Uint8Array) {
            ...
            recordset.rows[recordIndex][columnName] = Buffer.from(recordset.rows[recordIndex][columnName]).toString(
              'base64'
            );
          }
    ...
    if (command.connectionString.startsWith('postgres://')) command.postgreConnectionString = command.connectionString;
      ...
      command.postgreConnectionString =
        'postgres://' +
        command.connectionStringInfo.userId +
        ':' +
        command.connectionStringInfo.password +
        '@' +
        command.connectionStringInfo.host;
      if (command.connectionStringInfo.port != null)
        command.postgreConnectionString += ':' + command.connectionStringInfo.port;
